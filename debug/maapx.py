...
def download_granule_http(url: str, fname: str):
    """Submits a GET request and writes the content to a file."""
...
def download_granule(maap: MAAP, todir: str, granule: Granule) -> IOResultE[str]:
    ...
    url: str = granule["Granule"]["OnlineAccessURLs"]["OnlineAccessURL"][1]["URL"]
    ...
    logger.debug(f"Downloading granule {url} to directory {todir}")

    #return impure_safe(granule.getData)(todir)
    fname = os.path.join(todir, ur.split('/')[-1])
    return impure_safe(download_granule_http)(ur, fname)
